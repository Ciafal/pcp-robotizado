    ...
    try {
      ...
      if (records && records.length > 0) {
        ...
        return sortedRecords.map((r: any, idx: number) => ({
          ...
          item_type: r.item_type || 'PRODUCTION',
          test_programming_id: r.test_programming_id,
          test_code: r.test_code,
          is_origin_test_programming: Boolean(r.is_origin_test_programming),
          is_locked_externally: Boolean(r.is_locked_externally),
          test_technical_lead: r.test_technical_lead,
          test_objectives: r.test_objectives,
          material_code: r.material_code,
          material_description: r.material_description,
          family_code: r.family_code,
          steel_grade: r.steel_grade,
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        const payload = {
          ...
          test_programming_id: item.test_programming_id || '',
          test_code: item.test_code || '',
          is_origin_test_programming: Boolean(item.is_origin_test_programming),
          is_locked_externally: Boolean(item.is_locked_externally),
          test_technical_lead: item.test_technical_lead || '',
          test_objectives: item.test_objectives || [],
