      ...
      <DialogContent className="modal-analitico w-[min(96vw,1800px)] max-w-[min(96vw,1800px)] h-[94vh] max-h-[94vh] 2xl:h-[min(94vh,1100px)] 2xl:max-h-[min(94vh,1100px)] p-0 flex flex-col overflow-hidden">
        ...
        <div className="flex-none px-6 py-4 bg-gradient-to-r from-[#003870] to-[#004C97] text-white flex items-center justify-between gap-4 border-b border-[#002b55] shadow-sm">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge className="bg-white/20 hover:bg-white/30 text-white text-[10px] font-bold border-white/30">
              ...
              >
                Família: <strong className="text-white">{familia}</strong> &bull; Bitola:{' '}
                <strong className="text-white">{bitola}</strong> &bull; Origem:{' '}
                <strong className="text-white">{origemCarteira}</strong>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Badge
              className={`text-xs font-bold px-3 py-1 border shadow-sm ${resultadoTemporal.badgeCor.bg} ${resultadoTemporal.badgeCor.text} ${resultadoTemporal.badgeCor.border}`}
            >
        ...
        <div className="flex-none px-6 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between gap-2 flex-wrap text-xs text-slate-700">
          <div className="flex items-center gap-4 flex-wrap">
            <span className="font-semibold text-slate-900 flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#004C97]" /> Análise Temporal e Ficha
              Operacional
            </span>
            <span className="text-slate-500">
              ...
              >
                {resultadoTemporal.saldoAtualT.toFixed(2)} t
              </strong>
            </span>
            <span className="text-slate-500">
              Projetado:{' '}
              ...
              >
                {resultadoTemporal.saldoProjetadoT.toFixed(2)} t
              </strong>
            </span>
          </div>
          <div className="text-[11px] text-slate-500 font-mono">
            SAP RFC ZSD28C &bull; Sincronização em tempo real
          </div>
        </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center text-xs">
                  <div className="bg-white p-1.5 rounded border border-slate-200">
                    ...
                    <strong className="font-mono text-slate-800">
                      {(sdcItem.industrializacao_sdc?.destinada_t ?? sdcItem.programado_t).toFixed(
                        2,
                      )}{' '}
                      t
                    </strong>
                  ...
                  <div className="bg-white p-1.5 rounded border border-slate-200">
                    ...
                    <strong className="font-mono text-indigo-700">
                      {(
                        sdcItem.industrializacao_sdc?.em_processo_t ?? sdcItem.em_producao_t
                      ).toFixed(2)}{' '}
                      t
                    </strong>
                  </div>
                  <div className="bg-white p-1.5 rounded border border-slate-200">
                    ...
                    <strong className="font-mono text-emerald-700">
                      {(
                        sdcItem.industrializacao_sdc?.concluida_t ?? sdcItem.estoque_total_t
                      ).toFixed(2)}{' '}
                      t
                    </strong>
                  </div>
                ...
                <div className="mt-2 pt-2 border-t border-slate-200">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[11px] font-bold text-slate-800 flex items-center gap-1">
                      <ClipboardCheck className="w-3.5 h-3.5 text-[#004C97]" /> Requisitos MTO
                      (Ficha Canônica)
                    </span>
        ...
        <div className="flex-none px-6 py-3 bg-white border-t border-slate-200 flex items-center justify-between gap-2 shadow-sm min-h-[60px]">
          <div className="text-xs text-slate-500 flex items-center gap-2">
            <span>
              Fonte: <strong>SAP RFC ZSD28C</strong>
            </span>
            <span>&bull;</span>
            <span>
