...
import { WeeklyScheduleItem, OfficialMaterialOption, DAYS_OF_WEEK } from '@/types/weekly-schedule'
...
export const EditProductModal: React.FC<EditProductModalProps> = ({
  ...
  // 2. Estado do Modo de Programação (Quantidade vs Horário)
  ...
  // 3. Metadados e Classificação
  const [selectedDay, setSelectedDay] = useState<
    'SEG' | 'TER' | 'QUA' | 'QUI' | 'SEX' | 'SAB' | 'DOM'
  >('SEG')
  const [selectedShift, setSelectedShift] = useState<string>('T1_L1')
  ...
  // Inicializa com os dados do item
  useEffect(() => {
    if (item && isOpen) {
      const initialFam = item.family_code || ''
      setSelectedFamilyCode(initialFam)
      setQuantityInput(String(item.planned_quantity_tons || '100'))
      const startH = item.start_datetime ? item.start_datetime.split(' ')[1] || '06:00' : '06:00'
      const endH = item.end_datetime ? item.end_datetime.split(' ')[1] || '14:20' : '14:20'
      setStartTimeInput(startH)
      setEndTimeInput(endH)
      setSelectedDay(item.day_of_week)
      setSelectedShift(item.shift_code)
      setOrderType(item.order_type)
      setProductionOrder(item.production_order || '')
      setSalesOrderMto(item.sales_order_mto || '')
      setCustomerName(item.customer_name || '')
      setPcpNotes(item.pcp_notes || '')

      const match = officialMaterials.find(
        (m) => m.material_code.toUpperCase() === item.material_code.toUpperCase(),
      )
      if (match) {
        setSelectedMaterial(match)
        if (!initialFam && match.family_code) {
          setSelectedFamilyCode(match.family_code)
        }
      } else {
        setSelectedMaterial({
          material_code: item.material_code,
          material_name: item.material_description || item.material_code,
          family_code: item.family_code || 'GERAL',
          productivity_th: item.productivity_rate_th || 12.0,
          dimension_spec: item.dimensions || '50x50 mm',
          steel_grade: item.steel_grade || 'SAE 1020',
          default_order_type: item.order_type,
        })
      }
    }
  }, [item, isOpen, officialMaterials])

  // Famílias homologadas para a linha
  const homologatedFamilies = useMemo(() => {
    const map = new Map<string, { code: string; name: string; count: number }>()
    officialMaterials.forEach((m) => {
      const code = m.family_code || 'GERAL'
      const name = m.family_name || code
      if (!map.has(code)) {
  ...
  // Produtos filtrados estritamente pela família selecionada
  const filteredMaterials = useMemo(() => {
    if (!selectedFamilyCode) return []
    return officialMaterials.filter((m) => {
      const famMatch = (m.family_code || 'GERAL') === selectedFamilyCode
      ...
      return (
        ...
        (m.steel_grade && m.steel_grade.toLowerCase().includes(term)) ||
        (m.dimension_spec && m.dimension_spec.toLowerCase().includes(term))
      )
    })
  }, [officialMaterials, selectedFamilyCode, searchTerm])

  const handleFamilyChange = (newFamCode: string) => {
    setSelectedFamilyCode(newFamCode)
  ...
  // Produtividade Oficial da Ficha Mestra
  const materialCadence = useMemo(() => {
    ...
    return (
      ...
      selectedMaterial.productivity_th ||
      null
    )
  }, [selectedMaterial, lineOverview, officialMaterials])

  // Cálculo Temporal Bidirecional Unificado
  const calculationResult = useMemo(() => {
    if (!selectedMaterial || materialCadence === null || materialCadence <= 0) {
      return null
    }

    if (programBy === 'QUANTITY') {
  ...
  const handleConfirmSave = () => {
    ...
    const updated: WeeklyScheduleItem = {
      ...item,
      ...
      steel_grade: selectedMaterial.steel_grade || item.steel_grade || 'SAE 1020',
      dimensions: selectedMaterial.dimension_spec || item.dimensions || '50x50 mm',
      ...
      <DialogContent className="max-w-3xl bg-white text-slate-900 border-slate-300 shadow-2xl p-0 overflow-hidden max-h-[92vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-bold text-white flex items-center gap-2">
                Editar Programação — Item #{item.sequence_order} (Linha {lineCode})
              </DialogTitle>
              <p className="text-xs text-blue-100">
                Recálculo automático e recálculo da cadeia temporal
              </p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold text-slate-900 flex items-center gap-1.5 uppercase tracking-wider">
                ...
                1. Família de Produto * (Linha {lineCode})
              </label>
            ...
            <Select value={selectedFamilyCode} onValueChange={handleFamilyChange}>
              <SelectTrigger className="text-xs bg-white border-slate-300 h-10 font-medium text-slate-900">
                <SelectValue placeholder="Selecione a Família de Produto..." />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="pt-2">
              ...
              {!selectedFamilyCode ? (
                <div className="border border-dashed border-slate-300 rounded-lg p-4 text-center bg-white text-slate-400 text-xs">
                  Selecione uma família acima para liberar a lista de materiais oficiais.
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-40 overflow-y-auto border border-slate-200 rounded-lg p-1.5 bg-white">
                ...
                <SelectContent>
                  {(lineOverview?.shifts || []).map((s) => (
                    <SelectItem key={s.code} value={s.code} className="text-xs">
                      {WeeklyScheduleEngine.formatShiftDisplay(s.name, s.code, item.crew_name)} (
          ...
          <div className="border-t border-slate-200 pt-3 space-y-3">
            ...
            <div>
              <label className="text-[11px] text-slate-600 block mb-0.5">
                5. Observações do PCP / Instrução Operacional
              </label>
              <Textarea
