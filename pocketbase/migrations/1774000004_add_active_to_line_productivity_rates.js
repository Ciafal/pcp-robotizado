...
migrate((app) => {
  ...
  if (collection) {
    ...
    if (!existingField) {
      collection.fields.addAt(collection.fields.length, new BoolField({
        name: 'active',
        required: false,
      }))
      app.save(collection)
    ...
    try {
      app.db().newQuery('UPDATE line_productivity_rates SET active = 1 WHERE active IS NULL').execute()
    } catch (e) {
